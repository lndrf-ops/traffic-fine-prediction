...
def render(generative_results, synthetic_log, completed_cases):
    ...
    if not generative_results:
        st.warning("Generative quality report not found. Run `python src/bonus_generative_ai.py` first.")
        return
    ...
    st.markdown("---")
    st.write(
        "This tab shows the generative model quality metrics and lets you download the synthetic log "
        "and quality report generated by `src/bonus_generative_ai.py`."
    )

    # Download buttons
    dl1, dl2 = st.columns(2)
    if synthetic_log is not None:
        dl1.download_button(
            "Download synthetic CSV",
            data=synthetic_log.to_csv(index=False),
            file_name="synthetic_event_log.csv",
            mime="text/csv",
        )
    else:
        dl1.info("Synthetic event log not available.")

    dl2.download_button(
        "Download quality report",
        data=json.dumps(generative_results, indent=2),
        file_name="generative_quality_report.json",
    ...
    if real_lengths is not None and synth_lengths is not None and not synth_lengths.empty:
        ...
        fig_len = px.histogram(
            ...
            opacity=0.7,
            histnorm="probability",  # density plot
        )
