...
Code Quality & Appropriateness — High quality, correctly scoped, and human-readable. Code must include clear, meaningful comments where necessary. Strictly enforce the KISS principle (Keep It Simple, Stupid). Penalize over-engineering, unnecessary complexity, and overly "clever" but unreadable code. Reward implementations that do only what is strictly necessary to solve the problem, done well.
...
Evaluate against the KISS principle: code should be clean, readable, reasonably commented, and entirely free of over-engineered bloat.
